...
async function run() {
    ...
    const uri =
        "mongodb+srv://adminMongo:<password>@cluster0.aqn2xym.mongodb.net/?retryWrites=true&w=majority";
    ...
    try {
        const insertManyResult = await collection.insertMany(cryptos);
    ...
    try {
        ...
        await cursor.forEach(cryptos => {
            console.log(`${cryptos.name} has ${cryptos.value} as value and we have ${cryptos.amount} registered.`);
        });
        // add a linebreak
        console.log();
    } catch (err) {
